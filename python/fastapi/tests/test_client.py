...
from simplehook_fastapi.client import Connection

from .conftest import MockWSServer


class TestListenToWebhooks:
    def test_returns_noop_in_production(self, fastapi_app: FastAPI) -> None:
        # Arrange
        os.environ["FASTAPI_ENV"] = "production"

        # Act
        conn = listenToWebhooks(fastapi_app, "test_key")
    ...
    def test_returns_noop_in_production_via_env(self, fastapi_app: FastAPI) -> None:
        # Arrange
    ...
    def test_returns_noop_when_explicitly_disabled(self, fastapi_app: FastAPI) -> None:
        # Arrange
    ...
    def test_force_enable_overrides_production(
        self, fastapi_app: FastAPI, mock_ws_server: MockWSServer,
    ) -> None:
        # Arrange
        os.environ["FASTAPI_ENV"] = "production"
...
class TestListenerId:
    ...
    def test_listener_id_in_ws_url_via_shorthand(
        ...
        # Act
        conn = listenToWebhooks(fastapi_app, "test_key", "my-listener", {
            "server_url": mock_ws_server.url,
            "silent": True,
        })
        time.sleep(0.5)
...
class TestRequestForwarding:
    def test_forwards_post_request_to_fastapi(
        self, fastapi_app: FastAPI, mock_ws_server: MockWSServer,
    ) -> None:
        # Arrange
